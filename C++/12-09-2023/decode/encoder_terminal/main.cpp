...
std::bitset<4>* create_random_keys(std::bitset<4>* old_keys, int array_size);
...
int main(){
    ...
    std::bitset<4>* new_keys = create_random_keys(keys, array_size);
...
std::bitset<4>* create_random_keys(std::bitset<4>* old_keys, int array_size){
    ...
    for (int i = 0; i < array_size; i++){
        do {
            for (int j = 0; j < 4; j++){
                temp_num[j] = !old_keys[i][j]; // рандомный bool
            }
